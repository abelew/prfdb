...
foreach my $orf (keys %pars_data) {
    ...
    my @json_data = (
		     {label => "S1", data => $pars_data{$orf}->{S1}},
		     {label => "V1", data => $pars_data{$orf}->{V1}},
		     {label => "score", data => Pars_Score($pars_data{$orf}->{V1}, $pars_data{$orf}->{S1}),},
		     );
