...
sub UnWind {
  ...
  while ($state->{front_pos} < $state->{back_pos}) {
	  ...
	  if ($in_pattern->[$state->{current}] eq '.' and $state->{spaces} > $me->{max_spaces}) {
	      ...
	      if ($state->{out_pattern}->[$state->{current}] > 0 and 
		  $state->{positions_filled} > 0 and 
	  ...
	  elsif ($in_pattern->[$state->{current}] eq '.' and
		 $state->{spaces} <= $me->{max_spaces}) {
...
#		  print "TESTT: $state->{out_pattern}->[96] $state->{out_pattern}->[97] $state->{out_pattern}->[98]\n";
    ...
    if ($state->{out_pattern}->[$state->{current}] =~ /\d/ and
	$state->{out_pattern}->[$state->{current}] > -1) {
