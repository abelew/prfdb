...
sub Print_Single_Accession {
  ...
  while (my $slip_info = shift(@{$slipsite_information})) {
    ...
    if ($highlighted_slip eq $vars->{slipstart}) {
      $template->process('sliplist_highlighted.html', $vars) or print $template->error(), die;
    }
    else {
...
sub Print_Multiple_Accessions {
  ...
  foreach my $id (sort {$data->{$b}->{ss_count} <=> $data->{$a}->{ss_count}} keys %{$data}) {
    $vars->{id} = $data->{$id}->{id};
    ...
    $vars->{ss_count} = $data->{$id}->{ss_count};
...
#  unshift (@{$species}, 'All');
  ...
  $vars->{algorithm} = $cgi->popup_menu(-name => 'algorithm',
					-values => ['pknots','nupack'],
					-default => 'pknots');
  $vars->{filters} = $cgi->checkbox_group(-name => 'filters',
